...
export async function generateSticker(record: StickerRecord): Promise<StickerResult> {
  const generatedDirectory = path.join(generatedRoot, slugify(record.theme), slugify(record.description));
  const fileName = record.format === "gif" ? "result.gif" : "result.svg";
  const absolutePath = path.join(generatedDirectory, fileName);

  await mkdir(generatedDirectory, { recursive: true });

  if (record.format === "gif") {
